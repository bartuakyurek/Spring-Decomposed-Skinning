...
smplmodel = SMPLModel(model_path='./body_models/smpl/female/model.pkl',device=comp_device)
...
for femaleid in femaleids:
    ...
    for data_fname in tqdm(data_fnames):
        ...
        #base_name = os.path.basename(os.path.normpath(data_fname)) # Base name is "sid*_pid*_poses.npz"
        #sidpid = base_name[:-10] # Remove '_poses.npz' part from the file name
        sidpid = data_fname[23:-10]
