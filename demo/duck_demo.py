...
FNAME = "duck"      # For i/o files # ----------------------------------------------------  EDIT !
keyframe_poses = poses.duck_rig_pose  # --------------------------------------------------  EDIT !
helper_idxs = np.array([i for i in range(1, 6)])  # ----------------------------  EDIT !
...
for rep in range(N_REPEAT + N_REST):         # This can be refactored too as it's not related to render
    for pose_idx in range(n_poses-1): # Loop keyframes, this could be refactored.
        for frame_idx in range(FRAME_RATE):
            ...
            if MODE=="Rigid":
                skel_mesh_points = posed_locations[2:] 
                if RENDER_MESH: mesh_points = skinning.LBS_from_quat(V_rest, W, abs_rot_quat[1:], abs_trans[1:], use_normalized_weights=NORMALIZE_WEIGHTS) 
            else:
                ...
                M = inverse_kinematics.get_absolute_transformations(rest_bone_locations, posed_locations, return_mat=True, algorithm=ALGO)[1:]  
